...
def main():
    ...
    fileName = 'optimizationResults_unlimited'

    carPos = np.reshape(np.random.randint(0, gridSize[0], 2 * nCars), (nCars, 2))
    eventPos = np.reshape(np.random.randint(0, gridSize[0], 2*nEvents), (nEvents, 2))
    eventTimes = np.zeros_like(eventPos)
    eventStartTime = eventTimes[:, 0]
    eventEndTime = eventTimes[:, 1] + lengthSim

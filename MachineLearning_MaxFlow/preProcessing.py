...
def calc_expected_max_flow(car_positions, cdf_mat, n, tw, close_reward=80, cancel_pen=140, open_not_comm=5):
    all_rewards = []
    t0 = time.clock()
    for i in range(n):
...
def main():
    ...
    for t in range(0, data.shape[2] - PRED_LEN + 1):
        ...
        for j in range(CAR_PERMUTATION):
            expected_reward = calc_expected_max_flow(k_car_positions[j], tmp_mat, STOCHASTIC_ITERS, EVENT_TIME_WINDOW)
            item_dict[id] = (tmp_mat, k_car_positions[j], expected_reward)
            deb_dict[id] = {'time': t, 'bounds': (ROW_MIN, ROW_MAX, COL_MIN, COL_MAX), 'ncars': NUM_CARS}
            id += 1

    pickle.dump(item_dict, OUT_ITEMS_PATH)
    pickle.dump(deb_dict, OUT_DEBUG_PATH)
    return

...
def main():
    ...
    # create case vectors
    networksDict = {}
    itr = itertools.product(sequence_sizeVec, batch_sizeVec, hidden_sizeVec, lrVec, otVec)
    for i in itr:
        networkStr = 'seq_{0}_bs_{1}_hs_{2}_lr_{3}_ot_{4}'.format(i[0], i[1], i[2], i[3], i[4])
        networksDict[networkStr] = {'seq':i[0], 'bs': i[1], 'hs': i[2], 'lr': i[3], 'ot': i[4]}

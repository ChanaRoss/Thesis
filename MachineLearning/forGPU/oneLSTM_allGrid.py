...
class Model(nn.Module):
    def __init__(self, class_size, hidden_size, batch_size, sequence_size):
    ...
    def test_spesific(self, testLoader):
        ...
        for inputs, labels in testLoader:
            ...
            inputBatch = inputVar.view((-1, self.sequence_size)).to(device)
            testOut = self.forward(inputBatch).to(device)
            testOut = testOut.view(localBatchSize, self.class_size, grid_size).to(device)
            _, labTest = torch.max(torch.exp(testOut.data), 1)
            self.calcLoss(testOut, labVar)
            self.loss = self.loss / (localBatchSize * grid_size)  # normalizing loss results
...
def main():
    ...
    my_net = Model(classNum, hidden_size, batch_size, sequence_size)
    my_net.lstm = my_net.create_lstm(1)  # lstm receives one grid point , therefore one feature
    ...
    for numEpoch in range(num_epochs):
        ...
        for i, (input, labels) in enumerate(dataloader_uber_train):
            ...
            inputBatch = inputVar.view((-1, my_net.sequence_size))
            netOut = my_net.forward(inputBatch)
            netOut = netOut.view(local_batch_size, my_net.class_size, grid_size)
            _, labTrain = torch.max(torch.exp(netOut.data), 1)
            my_net.calcLoss(netOut, labVar)
            my_net.loss = my_net.loss/(local_batch_size*grid_size)  # normalizing loss results
            # backwards
            my_net.backward()
            # optimizer step

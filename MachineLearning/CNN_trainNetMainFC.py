...
itr = itertools.product(batchSize,conv1Vec,conv2Vec,conv3Vec,kernalSize,strideSize,paddingSize,
                        ...
                        optimType,lrVec,weightDecay,momentum,dampening,conv5Vec,epsilon,lrDecay,fcSize2,conv4Vec)
for i in itr:
    ...
    networksDict[networkStr] = {'bs': i[0], 'cnv1': i[1], 'cnv2': i[2], 'cnv3': i[3], 'ks': i[4], 'ss': i[5],'ps': i[6],
                                ...
                                'cnv5' : i[14],'eps':i[15],'lrd':i[16],'fc2':i[17],'cnv4':i[18]}
...
for netConfig in networksDict:
    ...
    for numEpoch in range(Net.maxEpochs):
        ...
        trainTot = 0.0
        if (1+numEpoch)%20 == 0:
            if Net.optimizer.param_groups[0]['lr']>0.001:
                Net.optimizer.param_groups[0]['lr'] = Net.optimizer.param_groups[0]['lr']/2
            else:
    ...
    pickle.dump(Net, open('NetObj_' + netConfig + '.pkl', 'wb'))

...
def main():
    ...
    for numEpoch in range(num_epochs):
        ...
        for i, (input, labels) in enumerate(dataloader_uber_train):
            ...
            batch_size_local = input.size(0)
            netOut   = torch.zeros([batch_size_local, classNum, x_size, y_size])
            for x in range(x_size):
                for y in range(y_size):  # calculate output for each grid_id
                    netOutTemp = my_net.forward(inputVar[:, :, :, :, k])
                    _, labTrain[:, x, y] = torch.max(netOutTemp.data, 1)
                    netOut[:, :, x, y] = netOutTemp
                    k += 1

...
def main():
    ...
    network_names = ['smooth_10_seq_30_bs_40_hs_128_lr_0.5_ot_1_wd_0.002_torch.pkl']
    ...
    timeIndexs = np.arange(200, 400, 1).astype(int)
    ...
    for network_name in network_names:
        ...
        for x in range(lengthX):
            for y in range(lengthY):
                for t, i in enumerate(timeIndexs):
                    ...
                    previousEventMatrix = getPreviousEventMat(dataInputReal[: , x, y].reshape([lengthT, 1, 1]), start_timeSm, my_net.sequence_size)
        ...
        for x in range(lengthX):
            for y in range(lengthY):
                ...
                plt.plot(timeIndexs, y_predAr[k*timeIndexs.size :(k+1)*timeIndexs.size], label='predicted output', linewidth=2, color = 'k')
                plt.legend()
                plt.xlabel('Time')

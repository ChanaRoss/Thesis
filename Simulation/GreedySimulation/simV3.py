...
def main():
    ...
    eventTemp = {'position': [], 'timeStart': 0, 'timeEnd': 0, 'closed': False, 'statusLog': [], 'id': 0, 'prob': 1}
    # params
    numEvents = 20
    ...
    if flagLoadParam == 1:
        ...
        locY   = gridHeight/2
        scaleY = gridHeight
        eventPosX = truncnorm.rvs((0-locX)/scaleX, (gridWidth-locX)/scaleX,  loc = locX, scale=scaleX, size=numEvents).astype(np.int64)
        eventPosY = truncnorm.rvs((0-locY)/scaleY, (gridHeight-locY)/scaleY, loc = locY, scale=scaleY, size=numEvents).astype(np.int64)
        # create event dict
    ...
    imageio.mimsave('./gif_HungarianMethod_log_Cost_WaitTime_CarMovement_' + str(gridHeight) + 'grid_' + str(numCars) + 'cars_' + str(lengthSim) + 'simLengh.gif', imageList, fps=1)
    # simulation summary
    plt.close('all')

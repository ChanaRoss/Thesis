...
from SolveDeterminsticAnticipatorySimulation_AStarV2 import *
sns.set()
...
def CalculateCostForKnownEventList(carDict,eventDict,timeLine,gridWidth,gridHeight):
    ...
    solver = SolveCars2EventsAstar(initial=initial, gridSize=gridHeight,maxTime = maxTime)
        ...
        if len(filteredEvents) == 0:
            ...
            for j,actions in enumerate(rt.product(actionOptions.values(), repeat=len(carDict))):
                ...
                for i in range(numStochasticEvents):
                    ...
                    tempCost = CalculateCostForKnownEventList(copy.deepcopy(carDictForCalc),copy.deepcopy(eventDictForCalc),timeLineForCalc,gridWidth,gridHeight)
                    if tempCost == 0:
                        tempCost = 0.001

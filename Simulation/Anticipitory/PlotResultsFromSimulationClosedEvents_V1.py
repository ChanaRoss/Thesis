    ...
    if 'AStar' in pickleName:
        ...
        for event in eventDict.values():
            if event['answered']:
                ...
                plt.scatter(event['position'][0], event['position'][1], color='b', label=event['id'])
                plt.text(event['position'][0], event['position'][1], event['id'])
        plt.xlabel('grid X')
        plt.ylabel('grid Y')
...
def main():
    ...
    for pickleName in pickleNames:
        ...
        if 'AStar' in pickleName:
            ...
            numEventsClosed = len([e for e in events.values() if e['answered'] and e['timeStart']+e['waitTime']<=simTime])

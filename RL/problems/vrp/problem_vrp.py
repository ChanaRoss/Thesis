...
from problems.vrp.state_mcvrp import StateMCVRP
...
class CVRP(object):
    ...
    def get_costs(dataset, pi):
        ...
        d = demand_with_depot.gather(1, pi)  # demand of each node gone to in the order of the policy
...
class MCVRP(object):
    ...
    NAME = 'mcvrp'  # Capacitated Vehicle Routing Problem

    VEHICLE_CAPACITY = 1.0  # (w.l.o.g. vehicle capacity is 1, demands should be scaled)

    @staticmethod
    def get_costs(dataset, pi):
        # size of pi in this case is : [n_cars, batch_size, max_sequence_size]
        batch_size, graph_size = dataset['demand'].size()
        n_cars = dataset['n_cars']
        d_tot = None
        for i in range(n_cars):
            pi_ = pi[i, ...]
            # Check that tours are valid, i.e. contain 0 to n -1
            sorted_pi = pi_.data.sort(1)[0]

            # Sorting it should give all zeros at front and then 1...n
            assert (
                torch.arange(1, graph_size + 1, out=pi_.data.new()).view(1, -1).expand(batch_size, graph_size) ==
                sorted_pi[:, -graph_size:]
            ).all() and (sorted_pi[:, :-graph_size] == 0).all(), "Invalid tour"
            ...
            d = demand_with_depot.gather(1, pi_)  # demand of each node gone to in the order of the policy

            used_cap = torch.zeros_like(dataset['demand'][:, 0])
            for i in range(pi_.size(1)):
                used_cap += d[:, i]  # This will reset/make capacity negative if i == 0, e.g. depot visited
                # Cannot use less than 0
                used_cap[used_cap < 0] = 0
                assert (used_cap <= CVRP.VEHICLE_CAPACITY + 1e-5).all(), "Used more than capacity"
            ...
            # Gather dataset in order of tour
            loc_with_depot = torch.cat((dataset['depot'][:, None, :], dataset['loc']), 1)
            d = loc_with_depot.gather(1, pi_[..., None].expand(*pi_.size(), loc_with_depot.size(-1)))
    ...
    @staticmethod
    def make_state(*args, **kwargs):
        return StateMCVRP.initialize(*args, **kwargs)
...
class VRPDataset(Dataset):
    ...
    def __init__(self, filename=None, size=50, num_samples=1000000, offset=0, distribution=None, n_cars=1):

...
class GraphAttentionDecoder(nn.Module):
    def __init__(
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            ...
            self.W_placeholder = nn.Parameter(torch.zeros(2 * embedding_dim))
    ...
    def forward(self, state, fixed, embeddings, car_id):
        n_nodes = embeddings.shape[1]
        # Compute keys, values for the glimpse and keys for the logits once as they can be reused in every step
        log_p = self._get_log_p(fixed, state, car_id).view(-1, n_nodes)
        return log_p

    def _get_log_p(self, fixed, state, car_id, normalize=True):

        # Compute query = context node embedding
        query = fixed.context_node_projected + \
                self.project_step_context(self._get_parallel_step_context(fixed.node_embeddings, state, car_id))

        # Compute keys and values for the nodes
        glimpse_K, glimpse_V, logit_K = self._get_attention_node_data(fixed, state)

        # Compute the mask
        mask = state.get_nodes_mask(car_id)
    ...
    def _get_parallel_step_context(self, embeddings, state, car_id, from_depot=False):
        """
        ...
        current_node_car = current_node[car_id, ...]
        first_a_car = state.first_a[car_id, ...]

        batch_size, num_steps = current_node_car.shape
            ...
            if num_steps == 1:  # We need to special case if we have only 1 step, may be the first or not
                if state.i.item() == 0:
                    ...
                    return embeddings.gather(
                        1,
                        torch.cat((first_a_car, current_node_car), 1)[:, :, None].expand(batch_size, 2,
                                                                                       embeddings.size(-1))
                    ).view(batch_size, 1, -1)
            # More than one step, assume always starting with first
            embeddings_per_step = embeddings.gather(
                1,
                current_node_car[:, 1:, None].expand(batch_size, num_steps - 1, embeddings.size(-1))
            )
            return torch.cat((

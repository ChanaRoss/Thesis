...
from SolveDeterminsticAnticipatorySimulation_AStarV3 import *
sns.set()
...
def CalculateCostForKnownEventList(carDict,eventDict,timeLine,gridWidth,gridHeight,weight):
    ...
    carsLoc = tuple([tuple(c['position']) for c in carDict.values()])
    eventsLoc = tuple([tuple(e['position']) for e in eventDict.values()])
    eventsTimeList = [e['timeStart'] for e in eventDict.values()]
    startTime = np.min(eventsTimeList)
    eventsTimeList = [e - startTime for e in eventsTimeList]
    maxTime = np.max(eventsTimeList)+3
    eventsTime = tuple(eventsTimeList)
    flagActions = 0
    initial = createInitialState(carsLoc, eventsTime, eventsLoc, flagActions)
    solver = SolveCars2EventsAstar(initial=initial, gridSize=gridHeight,maxTime = maxTime,weight = weight)
    astarSolution = astar_search(solver)
    TotalCost = astarSolution.path_cost
    numEventsClosed = len([e for e in astarSolution.state if isinstance(e,tuple) and 'event' in e and e[3]])
    numEvents = len(eventsLoc)
    eff = numEventsClosed/numEvents
...
def main():
    ...
    numCars = 2
    numEvents = 15
    lengthSim = 10  # minutes
    gridWidth = 3
    gridHeight = 3
    eventDuration = lengthSim
    lastEventDelta = 1
    aStarWeight = 2
    numStochasticEvents = 40
    # templates
    ...
    actionOptions = {'Up': [0, 1], 'Down': [0, -1], 'Left': [-1, 0], 'Right': [1, 0],'stay':[0,0]}
        ...
        if len(filteredEvents) == 0:
            ...
            for j,actions in enumerate(rt.product(actionOptions.values(), repeat=len(carDict))):
                ...
                for i in range(numStochasticEvents):
                    ...
                    tempCost = CalculateCostForKnownEventList(copy.deepcopy(carDictForCalc),copy.deepcopy(eventDictForCalc),timeLineForCalc,gridWidth,gridHeight,aStarWeight)
                    if tempCost == 0:
                        tempCost = 0.001
                ...
                if costOfAction > expectedCost + actionCost: # we want to minimize the expected cost of the plan since the cost is the movement of the cars and the time that each passanger waited
                    costOfAction = expectedCost + actionCost
